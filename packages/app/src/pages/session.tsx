...
import { createWorkspaceSessionController } from "@/pages/session/workspace-session"
...
export default function Page() {
  ...
  const workspace = createWorkspaceSessionController({
    sessionID: () => params.id,
    partMap: () => sync().data.part,
    syncTerminal: (open) => (open ? view().terminal.open() : view().terminal.close()),
  })

  const [codeView, setCodeView] = createSignal<"code" | "browser">("code")
  ...
  useSessionCommands({
    ...
    setWorkspaceMode: workspace.setMode,
    toggleWorkspaceTerminal: workspace.toggleTerminal,
    toggleCodeView: () => setCodeView(codeView() === "code" ? "browser" : "code"),
  })
          ...
          <WorkspacePane
            ...
            toolbar={
              <PaneToolbar
                ...
                onToggleFileTree={workspace.toggleFileTree}
                onToggleTerminal={workspace.toggleTerminal}
                fileTreeOpen={workspace.mode() === "command-center"}
                terminalOpen={workspace.mode() === "command-center"}
              />
            }
            fileTree={
              <Suspense>
            ...
            code={
              <div data-component="code-slot">
                <div data-slot="code-slot-tabs">
                  <Tooltip
                    placement="top"
                    value={
                      <span data-slot="code-slot-tooltip">
                        {language.t("ui.workspace.code")}
                        {command.keybind("workspace.codeView") ? (
                          <span data-slot="code-slot-keybind">{command.keybind("workspace.codeView")}</span>
                        ) : null}
                      </span>
                    }
                  >
                    <button
                      data-slot="code-slot-tab"
                      data-active={codeView() === "code" ? "true" : undefined}
                      aria-label={language.t("ui.workspace.code")}
                  ...
                  <Tooltip
                    ...
                    value={
                      <span data-slot="code-slot-tooltip">
                        {language.t("ui.workspace.browser")}
                        {command.keybind("workspace.codeView") ? (
                          <span data-slot="code-slot-keybind">{command.keybind("workspace.codeView")}</span>
                        ) : null}
                      </span>
                  ...
                  >
                    <button
                      data-slot="code-slot-tab"
                      data-active={codeView() === "browser" ? "true" : undefined}
                      aria-label={language.t("ui.workspace.browser")}
                      onClick={() => setCodeView("browser")}
                    >
                ...
                <div data-slot="code-slot-content">
                  <Show when={codeView() === "code"} fallback={<BrowserPreview />}>
                    <CodeWorkspace
                      tabs={workspace.tabs}
                      activeTab={workspace.activeId()}
                      onTabChange={workspace.setActiveId}
            ...
            terminal={<TerminalPanelV2 stacked />}
          />
        </Show>
