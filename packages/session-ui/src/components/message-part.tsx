...
import { BasicTool, GenericTool, resolveToolCategory } from "./basic-tool"
...
const agentTones: Record<string, string> = {
  ...
  planner: "var(--icon-agent-planner-base)",
  reviewer: "var(--icon-agent-reviewer-base)",
}
...
const v2AgentTones: Record<string, string> = {
  ...
  planner: "var(--v2-agent-planner-solid)",
  review: "var(--v2-agent-review-solid)",
  reviewer: "var(--v2-agent-reviewer-solid)",
        ...
        <Switch>
          ...
          <Match when={true}>
            <Dynamic
              ...
              category={resolveToolCategory(part().tool)}
              hideDetails={props.hideDetails}
              defaultOpen={props.defaultOpen}
              open={controlledOpen()}
              onOpenChange={props.onToolOpenChange ? handleToolOpenChange : undefined}
...
PART_MAPPING["reasoning"] = function ReasoningPartDisplay(props) {
  ...
  const status = createMemo(() => {
    if (streaming()) return "thinking" as const
    if (text()) return "completed" as const
